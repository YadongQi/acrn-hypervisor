...
static void tpm_crb_request_deliver(void *arg)
{
	...
	while (1) {
		...
		if (ret) {
			fprintf(stderr, "ERROR: Failed to wait condition(%d)\n", ret);
			break;
		}
...
static void crb_reg_write(void *vdev, uint64_t addr, int size, uint64_t val)
{
	...
	case CRB_REGS_CTRL_START_OFF:
		if ((val == CRB_CTRL_START_CMD) &&
			...
			tpm_vdev->cmd.out_len = TPM_CRB_MMIO_SIZE - CRB_DATA_BUFFER_OFF;

			pthread_mutex_lock(&tpm_vdev->request_mutex);
			pthread_cond_signal(&tpm_vdev->request_cond);
			pthread_mutex_unlock(&tpm_vdev->request_mutex);
		}
		break;
...
static int tpm_crb_reset(void *dev)
{
	...
	/* Emulator startup */
	if (swtpm_startup(TPM_CRB_MMIO_SIZE - CRB_DATA_BUFFER_OFF)) {
		fprintf(stderr, "Failed to startup TPM emulator!\n");
		return -1;
	}
...
void init_tpm_crb(struct vmctx *ctx)
{
	...
	assert(pthread_mutex_init(&tpm_vdev->request_mutex, NULL) == 0);
	assert(pthread_cond_init(&tpm_vdev->request_cond, NULL) == 0);
	assert(pthread_create(&tpm_vdev->request_thread, NULL, (void *)&tpm_crb_request_deliver, (void *)tpm_vdev)== 0);
	assert(pthread_setname_np(tpm_vdev->request_thread, "tpm_crb_request") == 0);
}
