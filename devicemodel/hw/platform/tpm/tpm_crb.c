...
struct tpm_crb_vdev {
	...
	pthread_t cmd_handle_thread;
	pthread_mutex_t cmd_handle_mutex;
	pthread_cond_t cmd_handle_cond;
};
...
static void tpm_crb_cmd_handler(void *arg)
{
	struct tpm_crb_vdev *tpm_vdev = (struct tpm_crb_vdev *)arg;
	int ret;

	while (1) {
		pthread_mutex_lock(&tpm_vdev->cmd_handle_mutex);

		pthread_cond_wait(&tpm_vdev->cmd_handle_cond, &tpm_vdev->cmd_handle_mutex);

		ret = swtpm_handle_request(&tpm_vdev->cmd);
		tpm_crb_request_completed(tpm_vdev, ret);

		pthread_mutex_unlock(&tpm_vdev->cmd_handle_mutex);
	}
}
...
static void crb_reg_write(void *vdev, uint64_t addr, int size, uint64_t val)
{
	...
	case CRB_REGS_CTRL_START_OFF:
		if ((val == CRB_CTRL_START_CMD) &&
			...
			tpm_vdev->cmd.out_len = tpm_vdev->be_buffer_size;

			pthread_mutex_lock(&tpm_vdev->cmd_handle_mutex);
			pthread_cond_signal(&tpm_vdev->cmd_handle_cond);
			pthread_mutex_unlock(&tpm_vdev->cmd_handle_mutex);
		}
		break;
...
void init_tpm_crb(struct vmctx *ctx)
{
	...
	assert(pthread_mutex_init(&tpm_vdev->cmd_handle_mutex, NULL) == 0);
	assert(pthread_cond_init(&tpm_vdev->cmd_handle_cond, NULL) == 0);
	assert(pthread_create(&tpm_vdev->cmd_handle_thread, NULL, (void *)&tpm_crb_cmd_handler, (void *)tpm_vdev)== 0);
	assert(pthread_setname_np(tpm_vdev->cmd_handle_thread, "tpm_crb_cmd") == 0);
}
...
void deinit_tpm_crb(struct vmctx *ctx)
{
	...
	pthread_cancel(tpm_vdev->cmd_handle_thread);
	pthread_cond_destroy(&tpm_vdev->cmd_handle_cond);
	pthread_mutex_destroy(&tpm_vdev->cmd_handle_mutex);
