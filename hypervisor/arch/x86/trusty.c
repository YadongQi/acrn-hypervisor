...
void save_world_ctx(struct acrn_vcpu *vcpu, struct ext_context *ext_ctx)
{
	...
	ext_ctx->vmx_cr0 = exec_vmread(VMX_CR0_MASK);
	ext_ctx->vmx_cr4 = exec_vmread(VMX_CR4_MASK);
...
void load_world_ctx(struct acrn_vcpu *vcpu, const struct ext_context *ext_ctx)
{
	...
	/* VMCS GUEST field */
	exec_vmwrite(VMX_CR0_MASK, ext_ctx->vmx_cr0);
	exec_vmwrite(VMX_CR4_MASK, ext_ctx->vmx_cr4);
...
void switch_world(struct acrn_vcpu *vcpu, int32_t next_world)
{
	...
	if (vcpu->vm->vm_id == 0) {
       ...
       {
               static int s_c = 0;
               if (vcpu->vm->vm_id == 0 && s_c <= 50) {
                       pr_err("SOS switch to World[%d](%d), rip=%llx!\n", next_world, s_c, arch->contexts[next_world].run_ctx.rip);
                       s_c ++;
               }
       }
